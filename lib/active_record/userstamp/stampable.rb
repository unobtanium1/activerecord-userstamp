...
module ActiveRecord::Userstamp::Stampable
  ...
  module ClassMethods
    ...
    def add_userstamp_associations(options)
      ...
      relation_options = options.reverse_merge(optional: true, class_name: klass)
      ...
      #fixed arity error (hash has no arity).. problem was belongs_to now takes scope as well so options 
      #were being passed in as hash and was being passed into proc check and arity was incorrectly called
      #solution:  ** derefence hash
      belongs_to :creator, **relation_options.reverse_merge(foreign_key: config.creator_attribute) if
        associations.first
      belongs_to :updater, **relation_options.reverse_merge(foreign_key: config.updater_attribute) if
        associations.second
